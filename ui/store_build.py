...
# Lấy đường dẫn gốc của dự án (Lùi 2 cấp nếu file này nằm ở ui/pages/)
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if os.path.basename(BASE_DIR).lower() == "ui":
...
class StorePage(QWidget):
    ...
    def get_launcher_version(self):
        """Đọc phiên bản Launcher từ file JSON cấu hình (ĐÃ SỬA CHUẨN ĐƯỜNG DẪN)"""
        possible_paths = [
            os.path.join(BASE_DIR, "config", "config.json"),    # <-- ĐÃ THÊM: Ưu tiên số 1
            os.path.join(BASE_DIR, "config", "version.json"),
            os.path.join(BASE_DIR, "config", "settings.json"),
            os.path.join(BASE_DIR, "version.json"),
            "config/config.json"                                # <-- ĐÃ THÊM: Backup đường dẫn tương đối
        ]
            ...
            if os.path.exists(path):
                try:
                    with open(path, "r", encoding="utf-8") as f:
                        data = json.load(f)
                        # Tìm các key version theo thứ tự ưu tiên
                        version = data.get("launcher_version") or data.get("version") or data.get("app_version")
                        if version:
                            return f"Vangrok Launcher v{version}"
                except Exception as e:
                    print(f"[STORE] Lỗi khi đọc file version tại {path}: {e}")
    ...
    def setup_ui(self):
        ...
        lbl_version.setStyleSheet("color: #888888; font-size: 13px; font-weight: bold; font-style: italic; background: transparent;")
        ...
        # ================= HỆ THỐNG BỘ LỌC ĐA NĂNG & SẮP XẾP =================
