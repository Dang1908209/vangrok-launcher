...
class GameDetailPage(QWidget):
    ...
    def setup_ui(self):
        ...
        # 1. Màn hình chính (Đã sửa: bỏ setFixedSize để màn hình tự co giãn linh hoạt)
        self.media_stack = QStackedWidget()
        self.media_stack.setMinimumSize(426, 240) # Giữ tỷ lệ tối thiểu 16:9
        ...
        # 2. Danh sách Thumbnail xếp dọc bên phải (Đã sửa: Bỏ setFixedSize chiều cao)
        self.scroll_area = QScrollArea()
        self.scroll_area.setFixedWidth(155)
        self.scroll_area.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Expanding)
        self.scroll_area.setWidgetResizable(True)
        self.scroll_area.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)

        self.thumbnails_container = QWidget()
        self.thumbnails_container.setStyleSheet("background: transparent;")
        ...
        # [ĐÃ SỬA LỖI UI BỊ LẸM CHỮ] - Gán tỷ lệ phần trăm thay vì fix cứng Pixel
        header_layout.addLayout(media_gallery_layout, stretch=6) # 60% chiều ngang cho Media
        header_layout.addLayout(info_layout, stretch=4)          # 40% chiều ngang cho Nút bấm
        ...
        self.desc_content_widget = QWidget()
        self.desc_content_widget.setStyleSheet("background: transparent;")
        
        self.desc_content_layout = QVBoxLayout(self.desc_content_widget)
        self.desc_content_layout.setContentsMargins(0, 0, 10, 0)
        self.desc_content_layout.setAlignment(Qt.AlignmentFlag.AlignTop)
        ...
        self.desc_content_layout.addWidget(self.detail_desc)
        self.desc_scroll.setWidget(self.desc_content_widget)
        
        desc_layout.addWidget(self.desc_scroll)
        # -------------------------------------------------------------

        layout.addWidget(self.desc_frame, stretch=1)

    # ==========================================
    # HÀM XỬ LÝ MỞ GAME TRỰC TIẾP
    # ==========================================
    ...
    def select_media(self, index):
        ...
        if yt_id or is_mp4:
            ...
            if WEB_ENGINE_AVAILABLE:
                if yt_id:
                    # Giả lập User Agent như một trình duyệt thật để không bị YouTube chặn
                    profile = self.video_player.page().profile()
                    ...
                    # [SỬA LỖI 153]: Bọc URL bằng thẻ <iframe> với referrerpolicy và setHtml kèm Base URL.
                    embed_html = f"""
                    <!DOCTYPE html>
                    <html style="width:100%; height:100%; margin:0; padding:0;">
                    <body style="width:100%; height:100%; margin:0; padding:0; background-color:black; overflow:hidden;">
                    ...
                    """
                    self.video_player.setHtml(embed_html, QUrl("https://localhost/"))
                else:
                    # [ĐÃ SỬA LỖI 1 NỬA] - Bắt buộc thêm thẻ <html height="100%"> và <body height="100%">
                    embed_html = f"""
                    <!DOCTYPE html>
                    <html style="width:100%; height:100%; margin:0; padding:0;">
                    <body style="width:100%; height:100%; margin:0; padding:0; background-color:black; display:flex; justify-content:center; align-items:center; overflow:hidden;">
                        <video style="width:100%; height:100%; outline:none;" controls autoplay>
                            <source src="{url}" type="video/mp4">
                        </video>
                    </body>
                    </html>
                    """
                    self.video_player.setHtml(embed_html, QUrl("http://localhost"))
        else:
            self.stop_video()
    ...
    def stop_video(self):
        if WEB_ENGINE_AVAILABLE and hasattr(self, 'video_player'):
            # [ĐÃ SỬA] Reset sạch sẽ bộ đệm webengine thay vì load khoảng trắng
            self.video_player.load(QUrl("about:blank"))
