...
class DevStatsPage(QWidget):
    ...
    def setup_ui(self):
        ...
        lbl_title.setStyleSheet(
            "font-size: 36px; font-weight: bold; color: white; letter-spacing: 2px; background: transparent;"
        )

        line = QFrame()
        line.setFrameShape(QFrame.Shape.HLine)
        line.setStyleSheet(
            "border-top: 2px solid #ff4d4d; max-width: 150px; margin-top: 5px; margin-bottom: 5px; background: transparent;"
        )

        lbl_desc = QLabel(
            "Real-time telemetry and management metrics of your game portfolio"
        )
        ...
        header_layout.addWidget(
            self.btn_refresh, alignment=Qt.AlignmentFlag.AlignVCenter
        )
        ...
        lbl_table_title.setStyleSheet(
            "font-size: 14px; font-weight: bold; color: white; background: transparent; margin-bottom: 5px;"
        )
        table_container_layout.addWidget(lbl_table_title)

        # Khởi tạo QTableWidget (ĐÃ TĂNG LÊN 6 CỘT ĐỂ THÊM CỘT LƯỢT TẢI)
    ...
    def calculate_and_populate_stats(self):
        ...
        games_list = (
            self.games_data if isinstance(self.games_data, list) else []
        )

        total_games = len(games_list)
        total_size_mb = 0
        total_downloads = 0  # Đã đổi thành biến lượt tải thật
        ...
        for index, game in enumerate(games_list):
            ...
            size_mb = 0
            size_val = game.get(
                "size_mb", game.get("size", game.get("file_size", 0))
            )
            try:
                ...
                if isinstance(size_val, str):
                    if "GB" in size_val.upper():
                        try:
                            size_mb = (
                                float(
                                    size_val.upper().replace("GB", "").strip()
                                )
                                * 1024
                            )
                        except:
                        ...
                        try:
                            size_mb = float(
                                size_val.upper().replace("MB", "").strip()
                            )
                        except:
                            pass
            ...
            # --- LẤY SỐ LƯỢT TẢI THẬT TỪ DATABASE ---
            # Ưu tiên lấy key "downloads", nếu không có tìm "download_count", mặc định là 0
            try:
                downloads = int(
                    game.get(
                        "downloads",
                        game.get(
                            "download_count", game.get("total_downloads", 0)
                        ),
                    )
                )
            except (ValueError, TypeError):
            ...
            item_name.setForeground(QColor("#ff4d4d"))
            self.table.setItem(row_position, 1, item_name)

            # 2. Ô Lượt tải thật (Mới thêm)
            item_downloads = QTableWidgetItem(f"{downloads} Lượt")
            item_downloads.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.table.setItem(row_position, 2, item_downloads)

            # 3. Ô Dung lượng
            display_size = (
                f"{int(size_mb)} MB"
                if size_mb < 1024
                else f"{round(size_mb/1024, 2)} GB"
            )
